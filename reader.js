...
const VALID_NUMBER = /^\d*\.?\d*/; // add at least decimals

const SINGLE_CHAR_TOKENS = ['(', ')', '{', '}', ',', ';', '+', '-', '*', '/', '=', '≠', '<', '>', '~', '&', '|']
const DUAL_CHAR_TOKENS = ['<=', '>='];
...
const read = (input = '', collector = []) => {
    ...
    } else if (DUAL_CHAR_TOKENS.includes(input.substring(0, 2))) {
        return read(input.substring(2), [...collector, tokens.get(input.substring(0, 2))]);

    } else if (SINGLE_CHAR_TOKENS.includes(input[0])) {
